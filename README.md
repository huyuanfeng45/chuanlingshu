...
1. 下载并安装传令书后，如果 macOS 提示无法确认开发者或拦截打开，请进入 `系统设置 > 隐私与安全性`，在安全提示里选择 `传令书`，然后点击“仍要打开”。
2. 打开传令书，在首次配置向导里填入飞书机器人的 `App ID` 和 `App Secret`，然后把这个机器人拉进你要使用的飞书群。
3. 在 Codex 里复制当前会话 ID，把这个会话 ID 发到飞书群里，并 `@机器人` 发送。传令书收到后会把这个飞书群和对应 Codex 会话绑定起来。
4. 如果你想在群里免 `@机器人` 直接说话，需要到飞书开放平台给应用添加“获取群组中所有消息”权限，也就是 `im:message.group_msg`；开通后重新发布应用，并确认机器人仍在群里。
...
- 支持飞书命令：`/help`、`/list`、`/use`、`/status`、`/watch`、`/看`、`/queue`、`/add`、`/bind`、`/bind-thread`、`/threads`、`/send-attachments`、`/cancel-attachments`。
...
- 任务卡支持按钮操作：刷新状态、线程截图、打开本机 Codex 线程、剩余用量、停止线程、线程引导、重新发送已完成/失败任务、追加队首、清空队列；“刷新状态”只更新任务文字进度，“线程截图”会单独截取当前 Codex 线程窗口并发送到飞书，“剩余用量”会把 Codex 的 5 小时和一周剩余用量推送到飞书。
- 支持接管电脑端已开始的任务：在飞书里发送 `/watch` 或 `/看`，会补发任务卡并继续同步当前 Codex 线程进度。
- 支持自动接管电脑端任务：在设置里开启后，已绑定飞书会话的 Codex 线程如果从电脑端手动开始新任务，会自动补发任务卡。
- 接管或运行任务时会临时保持 Mac 唤醒，任务结束后自动恢复系统睡眠策略。
...
- 在 Codex 界面输入模式下，飞书图片会作为 Codex 聊天框图片附件粘贴进去；聊天框文字只保留你在飞书里发送的原文，非图片文件仍会把本机路径发给 Codex 读取。
...
选择项目并绑定现有 Codex 线程后，直接发送普通文本、图片或文件即可继续对应 Codex 线程。飞书附件会保存到 Electron userData 下的 `feishu-attachments/日期/项目名/`。在 Codex 界面输入模式下，图片会像你手动在 Codex App 里插入图片一样粘贴到聊天框，文字只粘贴你在飞书里发送的原文；文件以及非界面输入模式仍会用本机绝对路径交给 Codex。
...
如果任务是你在电脑 Codex 里手动开始的，飞书群里还没有任务卡，可以在离开电脑前发送 `/watch` 或 `/看`。当前群已经绑定项目时可以不带参数；也可以发送 `/watch codex_thread_id` 或 `/看 codex_thread_id` 精确接管某个线程。传令书不会向 Codex 发送新消息，只会补发任务卡、回放最近会话日志并继续同步后续进度。

如果想省掉手动发送 `/watch`，可以在设置里开启“自动接管电脑端已开始的 Codex 任务”。这个开关默认关闭；开启后只对已经绑定飞书群/会话的 Codex 线程生效，未绑定线程不会自动推送到飞书。
...
任务卡“线程截图”按钮会打开对应 Codex 线程并截图发回飞书。这个截图能力还需要 macOS 屏幕录制权限：
