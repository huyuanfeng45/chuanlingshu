...
- 在 Codex 界面输入模式下，飞书图片会作为 Codex 聊天框图片附件粘贴进去；非图片文件仍会把本机路径发给 Codex 读取。
...
选择项目并绑定现有 Codex 线程后，直接发送普通文本、图片或文件即可继续对应 Codex 线程。飞书附件会保存到 Electron userData 下的 `feishu-attachments/日期/项目名/`。在 Codex 界面输入模式下，图片会像你手动在 Codex App 里插入图片一样粘贴到聊天框；文件以及非界面输入模式仍会用本机绝对路径交给 Codex。
